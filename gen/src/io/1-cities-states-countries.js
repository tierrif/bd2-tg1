...
export const insert = async (mssql, pool, j, total) => {
  ...
  for (const city of iterableJson) {
    ...
    const [country] = (await request.query(`SELECT * FROM Locations.Country WHERE Name = @country`)).recordset

    let identity
    if (!country) {
      const processingStr = `\r(${j}/${total}) Processing country: ${city.country} (${i++}/${244})...                                 `
      process.stdout.write(processingStr)
      identity = (await request.query(`INSERT INTO Locations.Country (Name) VALUES (@country); SELECT @@IDENTITY as countryId`)).recordset[0]
    } else {
      identity = (await request.query(`SELECT * FROM Locations.Country WHERE Name = @country`)).recordset[0]
    }
    ...
    if (city.subcountry) {
      ...
      const [state] = (await request.query(`SELECT * FROM Locations.State WHERE Name = @state`)).recordset

      if (!state) {
        await request.query('INSERT INTO Locations.State (Name, CountryId) VALUES (@state, @country_id)')
      }
    }

    request.input('name', mssql.NVarChar, city.name)
    request.query('INSERT INTO Locations.City (Name, CountryId) VALUES (@name, @country_id)')
  }
}
